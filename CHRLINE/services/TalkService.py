...
class TalkService(ChrHelperProtocol):
    ...
    def sendCompactMessage(
        self, to: str, text: Optional[str] = None, chunks: Optional[list] = None,
        *,
        stkid: Optional[str] = None, stkpkgid: Optional[str] = None
    ):
        ...
        if text is not None:
            cType = 2
        elif stkid is not None and stkpkgid is not None:
            cType = 4
        elif chunks:
            cType = 5
            ep = self.client.LINE_COMPACT_E2EE_MESSAGE_ENDPOINT
        else:
            raise ValueError("Unsupported message type for compact message.")
        ...
        if cType == 2:
            ...
            # NOTE: 目前已知此處需要 4 個 I32, 但實際上可能處於WIP狀態, 詳細參數不得而知
            raise NotImplementedError("Sticker is not supported for compact message yet.")
        elif cType == 5:
            sqrd += [2]
            for _ck in chunks[:3]:
    ...
    def getIncrementalBackupMessages(
        self,
        chatMid: str,
        messagesCount: int,
        inclusive: bool,
        endMessageId: int,
        endMessageDeliveredTime: int,
    ):
        """Get incremental backup messages."""
        # CREATED: 10/31/2024, 14:52:45
        METHOD_NAME = "getIncrementalBackupMessages"
        _endMessageId = [
            [10, 1, endMessageDeliveredTime],
            [10, 2, endMessageId],
        ]
        request = [
            [11, 1, chatMid],
    ...
    def updatePinState(self, chatMid: str, isPinned: bool):
        """Update pin state."""
        # CREATED: 10/31/2024, 14:52:45
        # FIXME: DEVKIT maybe, not found in ANDROID v26.4.0
    ...
    def getBackupChats(self):
        ...
        """
        # CREATED: 10/31/2024, 14:52:45
        # 2026/05/16: Invalid method name: 'getBackupChats' (removed)
    ...
    def editMessage(self):
        """Edit message."""
        # CREATED: 05/16/2026, 04:36:00
        raise Exception("editMessage is not implemented")
